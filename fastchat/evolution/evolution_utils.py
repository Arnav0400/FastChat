...
sys.path.append('/home/arnav.chavan/NIPS23/lm-evaluation-harness')
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, choices, output_dir):
        ...
        self.cand_params = []
        self.model = lm_eval.models.get_model('hf-causal-experimental').create_from_arg_string(
            "pretrained=meta-llama/Llama-2-7b-hf,peft=/l/users/arnav.chavan/checkpoints_glora_llama2_BS=32_R=4", {"batch_size": 'auto', "device": 'cuda'}
        )
        self.task_dict = lm_eval.tasks.get_task_dict([f"hendrycksTest-{sub}" for sub in SUBJECTS]+["hellaswag", "truthfulqa_mc", "arc_challenge"])
    ...
    def evaluate(self, config):
        ...
        acc += results['results'][f"hellaswag"]['acc_norm']
        acc += results['results'][f"truthfulqa_mc"]['mc2']
        acc += results['results'][f"arc_challenge"]['acc_norm']
        print('Candidate Accuracy:',acc/4)
        return acc/4
